    ...
    def lengthOfLongestSubstring(self, s: str) -> int:
        ...
        for r in range(len(s)):
            while s[r] in char_set:
                ...
                l += 1
            char_set.add(s[r])
            res = max(res, r - l + 1)  # r - l + 1 can also be represented by len(char_set)
                                       # but using pointers l and r directly performs faster
        return res

"""
return LENGTH of longest substring
NO duplicate chars, use set?
...
T: O(N)  # N = total number of chars in the string
S: O(M)  # M = total number of unique chars in the string
